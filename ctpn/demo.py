...
def draw_boxes(img,image_name,boxes,scale):
    ...
    with open('/content/drive/My Drive/GR2/ctpn/split_line/results/' + '{}.txt'.format(base_name.split('.')[0]), 'w') as f:
        for box in boxes:
    ...
    print('===== dump file ====',os.path.join("/content/drive/My Drive/GR2/ctpn/split_line/results_visualize/", base_name))
    cv2.imwrite(os.path.join("/content/drive/My Drive/GR2/ctpn/split_line/results_visualize/", base_name), img)
...
def ctpn(sess, net, image_name):
    ...
    scores, boxes,mask,blobs = test_ctpn(sess, net, img)
    ...
    print(visualize_map.shape,'1')
    visualize_map=np.clip(visualize_map,0,254).astype(np.uint8)
    print(visualize_map.shape)
    cv2.imwrite("/content/debug/mask_"+os.path.basename(image_name),visualize_map[0])
