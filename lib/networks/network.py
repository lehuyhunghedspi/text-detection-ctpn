...
class Network(object):
    ...
    @layer
    def conv(self, input, k_h, k_w, c_o, s_h, s_w, name, c_i=-1,biased=True,relu=True, padding=DEFAULT_PADDING, trainable=True):
    ...
    def transpose_conv(self,input,k_h,k_w,c_i,c_o,s_h, s_w, name, biased=True,relu=True, padding=DEFAULT_PADDING, trainable=True):
        ...
        with tf.variable_scope(name) as scope:
            ...
            init_weights = tf.truncated_normal_initializer(0.0, stddev=0.01)
            init_biases = tf.constant_initializer(0.0)
            print([k_h, k_w, c_i, c_o])
            kernel = self.make_var('weights', [k_h, k_w,  c_o,c_i], init_weights, trainable, \
                                   regularizer=self.l2_regularizer(cfg.TRAIN.WEIGHT_DECAY))
            if biased:
                biases = self.make_var('biases', [c_o], init_biases, trainable)
                print("======== bug ====")
    ...
    def build_loss(self, ohem=False):
        ...
        return total_loss,model_loss, rpn_cross_entropy, rpn_loss_box, mask_loss
